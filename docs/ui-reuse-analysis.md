...
**Date:** 2026-09-08  
**Goal:** Windows desktop app `Karrierekrake.exe` with maximum reuse, minimum new code.
...
| Karrierekrake (current) | this repo | **PySide6 desktop** (`desktop/`) | PyInstaller onefile EXE |
...
| ------------------- | --------- | -------- | --------- | --------------------- |
| Dashboard | Full bot dashboard (start/pause, stats, feed, review card) | Stats + top matches | **UX ideas only** | Full rewrite for Karrierekrake API/DB |
...
| Profile | Experience files / KB-oriented | Read-only CV extract display | **No meaningful code reuse** | Need editable Karrierekrake YAML fields |
...
| First-run Wizard | Yes (~7 steps) | No | Pattern only | Rebuild for Karrierekrake defaults |
...
- Existing Karrierekrake backend (`core/`, `search/`, `apply/`, `browser/`, `app.main.run_pipeline`)
